        ...
        if whi.data['change_type']:
            if whi.data['change_type'] == '修改离职时间':
                ...
                if whi.data['original_resignation_y_n'] != whi.data['modify_resignation_y_n']:
                    ...
                    res_o_r_y_n, err = await jd_attendance_record_form.get_all_data(
                        data_filter={
                            "cond": [
                                {
                    ...
                    for value_res_o_r_y_n in res_o_r_y_n:
                        _, err = await jd_attendance_record_form.update_data(
                    ...
                    res_m_r_y_n, err = await jd_attendance_record_form.get_all_data(
                        data_filter={
                            "cond": [
                                {
                                    "field": 'worknumberzd',  # 终端工号
                    ...
                    # 一个月内
                    res2, err = await jd_attendance_record_form.get_all_data(
                        data_filter={
                            "cond": [
                                {
                                    "field": 'worknumberzd',  # 终端工号
