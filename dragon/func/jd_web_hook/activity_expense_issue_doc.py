...
def register(router: APIRouter):
    @router.post('/activity-expense-issue-doc', tags=['活动费用出库单-创建数据到->营销_产品出库总表'], description=doc)
    async def activity_expense_issue_doc(whi: WebHookItem, req: Request, background_tasks: BackgroundTasks):
        # 验证签名
        ...
        if whi.data['total_issue_quantity'] != 0:
            _, err = await jd.query_update_data_one(
                ...
                data={
                    ...
                    'delivery_details':
                        JDSerialize.subform(subform_field="delivery_details", data=whi.data['delivery_details'])[
                            "delivery_details"],
                    # 出库明细 子表单
