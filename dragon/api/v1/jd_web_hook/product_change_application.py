...
async def business(whi):
    ...
    if whi.data['flowState'] == 1 and whi.op == 'data_update':
        ...
        # 客户档案
        widgets = await customer_profile.get_form_widgets()
            ...
            if val['label'] == '客户名称' and val['type'] == 'text':
                ...
                for items in val['items']:
                    if items['label'] == '产品状态':
                        ...
                        # 过渡表
        widgets = await customer_sales_product_table.get_form_widgets()
        ...
        # 客户档案 -> 查询表单多条数据
        result = await customer_profile.get_form_data(
            data_filter={
                "cond": [
                    {
        ...
        whi_data_subform = whi.data[whi_data_subform_field]  # 临时 新增或修改产品信息申请->销售产品（子表单）
        ...
        for ii in range(len(result)):
            for n in range(len(whi_data_subform)):
                result2 = await customer_sales_product_table.get_form_data(
                    data_filter={
                        "cond": [
                            {
                ...
                if not result2:
                    await customer_sales_product_table.create_data(
                        data={
                            # 客户代码
                            sales_ccode_field: {
                    ...
                    await customer_sales_product_table.update_data(
